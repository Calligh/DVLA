		 	...
		 	<?php 
				...
				if ($query_count > 0) {
					...
					while ($row = $query_data->fetch_array(MYSQLI_BOTH)) 
					{
						...
						$lessons 	  = $row['lessons']; // this is the foreign key constrain
						$menu_fk_id = $row['menu_id'];
		 	 ?>
			 		<div class="card-panel">
			 			<span style="text-transform: capitalize;font-size: 18px;"><a href="user/featured/<?php echo "$lessons"; ?>/<?php echo "$lesson_id"; ?>/<?php echo "$menu_fk_id"; ?>"><?php echo "$lesson_title"; ?></a></span>
			 			...
			 			<ul class="dropdown-content detail_dropdown" id="dropdown<?php echo "$lesson_id";?>">
						 	<li class="waves-effect waves-ripple"><a href="user/featured/<?php echo "$lessons"; ?>/<?php echo "$lesson_id"; ?>/<?php echo "$menu_fk_id"; ?>"><i class="material-icons">visibility_on</i>Read ..</a></li>
						    <li class="divider"></li>
